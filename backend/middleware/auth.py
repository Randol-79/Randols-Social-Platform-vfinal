...
class AuthConfig:
    ...
    ADMIN_API_KEYS = set(filter(None, [
        os.getenv('ADMIN_API_KEY'),
    ]))
...
def validate_api_key(api_key: str) -> tuple[bool, bool]:
    ...
    is_valid = any(
        ...
        for valid_key in AuthConfig.VALID_API_KEYS
    )
    
    is_admin = any(
        hmac.compare_digest(api_key, admin_key)
        for admin_key in AuthConfig.ADMIN_API_KEYS
    )
