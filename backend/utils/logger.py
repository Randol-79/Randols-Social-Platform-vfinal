...
class AgentLogger:
    ...
    def content_event(self, event_type: str, content_id: str, **kwargs):
        ...
        data = {"event_type": event_type, "content_id": content_id, **kwargs}
        self._log_with_context(logging.INFO, message, data)


def setup_logger(
    name: str, level: int = logging.INFO, log_to_file: bool = True, json_format: bool = False
) -> AgentLogger:
    """
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.info(
                    f"Completed {func.__name__}", duration_seconds=duration, status="success"
                )
                return result
