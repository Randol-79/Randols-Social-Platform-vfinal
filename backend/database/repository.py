...
from .models import (
    ...
    ContentInDB, ContentStatus, Platform, ContentType,
    ScheduledPost, ContentCalendar, DailyAnalytics, WeeklyReport,
    AgentLog, AgentState, ABTest, AuditLog, EmergencyOverride,
...
class ContentRepository:
    ...
    async def get_for_platform(self, platform: Platform, status: ContentStatus = None, limit: int = 50) -> List[Dict]:
...
class AnalyticsRepository:
    ...
    async def get_weekly_reports(self, limit: int = 12) -> List[Dict]:
        ...
        cursor = self.weekly_collection.find().sort(
...
class AgentRepository:
    ...
    async def get_logs(self, agent_name: str = None, limit: int = 100) -> List[Dict]:
        ...
        cursor = self.log_collection.find(query).sort(
...
class NotificationRepository:
    ...
    async def get_unread(self, limit: int = 50) -> List[Dict]:
        ...
        cursor = self.collection.find({"read": False}).sort(
            "sent_at", DESCENDING
        ).limit(limit)
