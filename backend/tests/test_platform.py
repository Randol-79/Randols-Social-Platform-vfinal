...
class TestAPIEndpoints:
    ...
    def test_content_generation(self, client):
        ...
        response = client.post(
            ...
            json={"type": "morning_greeting", "platforms": ["instagram", "facebook"]},
...
class TestIntegration:
    ...
    async def test_content_pipeline(self, sample_context):
        ...
        # Validate
        guardian = BrandVoiceGuardianAgent()
        sample_content = {"id": "test", "text": "Come join us for authentic Cajun cuisine, cher!"}
        ...
        if is_valid:
            result = await scheduler.schedule_content(
                sample_content, datetime.now() + timedelta(hours=1)
            )
            assert result["success"]
