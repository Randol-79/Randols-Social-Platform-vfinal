...
class SchedulerAgent:
    ...
    def __init__(self):
        ...
        self.platform_post_counts = defaultdict(lambda: {'daily': 0, 'hourly': 0, 'last_hour': None})
    
    async def schedule_content(self, content: Dict[str, Any], scheduled_time: Optional[datetime] = None) -> Dict[str, Any]:
    ...
    def _calculate_optimal_time(self, content: Dict[str, Any]) -> datetime:
        ...
        optimal_time = now.replace(hour=pref['hour'], minute=pref['minute'], second=0, microsecond=0)
        ...
        platform_adjustments = self._get_platform_optimal_adjustment(platforms[0] if platforms else 'instagram')
        ...
        for platform in platforms:
            ...
            if counts['daily'] >= limits.get('daily', 999):
                return {
                    ...
                    'suggested_time': (now + timedelta(days=1)).replace(hour=9, minute=0).isoformat()
                }
    ...
    async def reschedule_post(self, post_id: str, new_time: datetime) -> Dict[str, Any]:
        ...
        if post['status'] not in ['scheduled', 'retry_scheduled', 'failed']:
            return {'success': False, 'error': f"Cannot reschedule post with status: {post['status']}"}
    ...
    def get_optimal_times(self, platform: str, content_type: str) -> List[Dict]:
        ...
        optimal_times = {
            'instagram': {
                'morning_greeting': [{'time': '08:00', 'score': 0.85}, {'time': '08:30', 'score': 0.82}],
                'daily_special': [{'time': '11:30', 'score': 0.90}, {'time': '12:00', 'score': 0.87}],
                'evening_event': [{'time': '17:00', 'score': 0.88}, {'time': '17:30', 'score': 0.85}],
                'general': [{'time': '12:00', 'score': 0.80}, {'time': '18:00', 'score': 0.78}]
            },
            'facebook': {
                'morning_greeting': [{'time': '08:30', 'score': 0.82}, {'time': '09:00', 'score': 0.80}],
                'daily_special': [{'time': '11:00', 'score': 0.88}, {'time': '11:30', 'score': 0.86}],
                'evening_event': [{'time': '16:30', 'score': 0.85}, {'time': '17:00', 'score': 0.83}],
                'general': [{'time': '13:00', 'score': 0.78}, {'time': '19:00', 'score': 0.76}]
            },
            'tiktok': {
                'general': [{'time': '19:00', 'score': 0.92}, {'time': '21:00', 'score': 0.90}],
                'daily_special': [{'time': '12:00', 'score': 0.85}, {'time': '18:00', 'score': 0.88}]
            }
        }
...
async def run_scheduler_loop(scheduler: SchedulerAgent, platform_manager, interval_seconds: int = 60):
