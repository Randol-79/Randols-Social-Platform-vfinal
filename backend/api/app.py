...
def generate_content():
    ...
    try:
        ...
        agents = get_agents()
        content = run_async(agents['content_generator'].generate_content_by_type(content_type, context))
        
        if content:
            is_approved = run_async(agents['brand_guardian'].validate_content(content))
            content['brand_approved'] = is_approved
            content['authenticity_score'] = agents['brand_guardian'].calculate_authenticity_score(content.get('text', ''))
...
def validate_content():
    ...
    try:
        ...
        agents = get_agents()
        is_approved = run_async(agents['brand_guardian'].validate_content({'text': text, 'id': 'validation_request'}))
...
def emergency_override():
    ...
    try:
        ...
        socketio.emit('emergency_override', {
...
def emergency_post():
    ...
    try:
        ...
        agents = get_agents()
        emergency_content = run_async(agents['content_generator'].generate_emergency_content({
            'reason': 'manual_emergency_post',
            'message': message,
            'platforms': [platform]
        }))
        
        socketio.emit('emergency_post_created', {
