...
def _check_redis_health() -> str:
    ...
    try:
        ...
        r = redis.Redis(
            host=Config.REDIS_HOST, port=Config.REDIS_PORT, password=Config.REDIS_PASSWORD or None
        )
        r.ping()
        return "healthy"
    except:
...
async def list_content():
    ...
    return jsonify(
        {
            "content": content,
            "pagination": {"limit": limit, "offset": offset, "total": len(content)},
        }
    )
...
async def create_content():
    ...
    if data.get("auto_validate", True):
        ...
        if is_valid:
            ...
            content["validation"] = {"issues": guardian.suggest_improvements(content["text"])}
...
async def get_schedule():
    ...
    return jsonify({"date": date, "posts": posts, "total": len(posts)})
...
async def get_pending_posts():
    ...
    return jsonify({"pending_posts": posts, "total": len(posts)})
...
async def get_calendar():
    ...
    for i in range(days):
        ...
        posts = await repo.get_for_date(date)
        calendar[date] = {"posts": posts, "total": len(posts), "by_platform": {}}
...
async def get_platform_analytics(platform: str):
    ...
    return jsonify({"platform": platform, "period_days": days, "summary": summary})
...
async def list_agents():
    ...
    for name in default_agents:
        if name in agent_map:
            ...
            agents.append({"agent_name": name, "status": "active", "health": "good"})
...
async def get_agent_status(agent_name: str):
    ...
    return jsonify({"agent_name": agent_name, **status})
...
async def get_agent_logs(agent_name: str):
    ...
    return jsonify({"agent_name": agent_name, "logs": logs, "total": len(logs)})
...
async def get_error_logs():
    ...
    return jsonify({"period_hours": hours, "errors": errors, "total": len(errors)})
...
async def pause_system():
    ...
    return jsonify({"message": "System paused", "status": orchestrator.get_system_status()})
...
async def resume_system():
    ...
    return jsonify({"message": "System resumed", "status": orchestrator.get_system_status()})
...
async def run_workflow():
    ...
    return jsonify({"message": "Workflow executed", "posts_created": len(posts), "posts": posts})
...
async def start_ab_test():
    ...
    return jsonify({"test_id": test_id, "message": "A/B test started"}), 201
